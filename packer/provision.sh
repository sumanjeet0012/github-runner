...
# Run as root.
# =============================================================================
# set -eu: exit on error and unset vars.
# Deliberately NOT using pipefail — curl|jq and similar pipes would trigger
# SIGPIPE (exit 141) when jq exits after reading the first value, killing the
# entire script. Individual critical pipes use explicit error checking instead.
set -eu
...
# ─────────────────────────────────────────────────────────────
log "3/17  Docker CE + Buildx + Compose"
...
apt-get update -y
apt-get install -y -o Dpkg::Options::="--force-confnew" -o Dpkg::Options::="--force-confdef" gh

gh --version

# ─────────────────────────────────────────────────────────────
# 5. BuildKit config (enables docker.io mirror to avoid rate limits)
# ─────────────────────────────────────────────────────────────
log "5/17  BuildKit config"
...
# ─────────────────────────────────────────────────────────────
# 7. Node.js (via NodeSource)
# ─────────────────────────────────────────────────────────────
log "7/17  Node.js ${NODE_VERSION}"
...
# ─────────────────────────────────────────────────────────────
# 8. uv (fast Python package/project manager)
# ─────────────────────────────────────────────────────────────
log "8/17  uv (Astral)"
...
# ─────────────────────────────────────────────────────────────
# 9. Python versions via uv (3.10 – 3.13)
# ─────────────────────────────────────────────────────────────
log "9/17  Python 3.10 – 3.13 via uv"
...
# ─────────────────────────────────────────────────────────────
log "10/17  Rust (${RUST_TOOLCHAIN} + MSRV ${RUST_MSRV})"
...
# ── Install cargo tools via cargo-binstall (pre-built binaries, ~2 min vs ~35 min) ──
# cargo-binstall itself is installed first from its own binary release.
CARGO_BINSTALL_VER=$(curl -fsSL https://api.github.com/repos/cargo-bins/cargo-binstall/releases/latest \
  | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
curl -fsSL \
  "https://github.com/cargo-bins/cargo-binstall/releases/download/v${CARGO_BINSTALL_VER}/cargo-binstall-x86_64-unknown-linux-musl.tgz" \
...
ln -sf /usr/local/cargo/bin/cargo-binstall /usr/local/bin/cargo-binstall
echo "[cargo-binstall] $(cargo-binstall --version)"

# Install cargo tools as pre-built binaries (no compilation needed)
cargo-binstall --no-confirm wasm-pack@0.12.0
cargo-binstall --no-confirm tomlq
...
# ─────────────────────────────────────────────────────────────
log "11/17  Nim (binary tarball, stable)"
...
# ─────────────────────────────────────────────────────────────
log "13/17  Shadow simulator compile-time deps"
...
#     Download the official Temurin tarball directly from GitHub releases.
#     This avoids the Adoptium APT repo which has intermittent 503 failures.
# ─────────────────────────────────────────────────────────────
log "14/17  Java (Temurin JDK ${JAVA_VERSION})"

# Fetch the latest Temurin 11 release tag from GitHub API
TEMURIN_TAG=$(curl -fsSL \
  "https://api.github.com/repos/adoptium/temurin${JAVA_VERSION}-binaries/releases/latest" \
  | grep '"tag_name"' | head -1 | sed 's/.*"\(jdk[^"]*\)".*/\1/')
echo "[Java] Temurin tag: ${TEMURIN_TAG}"

# Build the download URL (format: OpenJDK11U-jdk_x64_linux_hotspot_<version>.tar.gz)
TEMURIN_VER=$(echo "${TEMURIN_TAG}" | sed 's/jdk-//' | sed 's/+/_/')
TEMURIN_URL="https://github.com/adoptium/temurin${JAVA_VERSION}-binaries/releases/download/${TEMURIN_TAG}/OpenJDK${JAVA_VERSION}U-jdk_x64_linux_hotspot_${TEMURIN_VER}.tar.gz"
echo "[Java] Downloading: ${TEMURIN_URL}"

curl -fsSL --retry 3 --retry-delay 5 "${TEMURIN_URL}" -o /tmp/temurin.tar.gz
tar -xzf /tmp/temurin.tar.gz -C /usr/local/
rm /tmp/temurin.tar.gz

# Move to a fixed path
JDK_DIR=$(find /usr/local -maxdepth 1 -type d -name 'jdk-11*' | head -1)
mv "${JDK_DIR}" /usr/local/temurin-11

# Symlink java/javac into /usr/local/bin
ln -sf /usr/local/temurin-11/bin/java   /usr/local/bin/java
ln -sf /usr/local/temurin-11/bin/javac  /usr/local/bin/javac
ln -sf /usr/local/temurin-11/bin/jar    /usr/local/bin/jar

java -version

# Set JAVA_HOME for all users
cat > /etc/profile.d/java.sh << 'JAVAEOF'
export JAVA_HOME=/usr/local/temurin-11
export PATH="$JAVA_HOME/bin:$PATH"
JAVAEOF
chmod 644 /etc/profile.d/java.sh
grep -q "JAVA_HOME" /etc/environment || echo "JAVA_HOME=/usr/local/temurin-11" >> /etc/environment

# ─────────────────────────────────────────────────────────────
# 14. Runner user + permissions  (created BEFORE runner download
#     so the runner dir is owned correctly from the start)
# ─────────────────────────────────────────────────────────────
log "15/17  Runner user '${RUNNER_USER}'"
...
# ─────────────────────────────────────────────────────────────
log "16/17  GitHub Actions Runner binary"
...
# ─────────────────────────────────────────────────────────────
log "17/17  Pre-baking runner scripts + systemd unit"
